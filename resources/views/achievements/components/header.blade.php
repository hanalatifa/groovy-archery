<section class="relative min-h-screen overflow-hidden bg-white">
        ...
        <div class="max-w-2xl py-32">
            ...
            <div class="hero-fade flex items-center gap-3 mb-10">
                ...
                <span class="text-[10px] font-bold uppercase tracking-[5px] text-gray-300">
                    {{ __('achievements.hero_label') }}
                </span>
            </div>

            <h1 class="hero-fade hero-delay-1 font-extrabold leading-[1.05] text-white mb-8"
                style="font-size: clamp(2.5rem, 6vw, 5rem);">
                {{ __('achievements.hero_title_1') }}<br>
                <span class="relative inline-block">
                    {{ __('achievements.hero_title_2') }}
                    <svg class="absolute -bottom-2 left-0 w-full" height="6" viewBox="0 0 200 6">
                        <line x1="0" y1="3" x2="200" y2="3" stroke="#DA4139"
                            stroke-width="2.5" stroke-linecap="round" />
                    </svg>
                </span><br>
                {{ __('achievements.hero_title_3') }}
            </h1>

            <p
                class="hero-fade hero-delay-2 text-gray-300 text-base md:text-lg leading-relaxed font-light max-w-xl mb-12">
                {{ __('achievements.hero_sub') }}
            </p>
            ...
            <div class="hero-fade hero-delay-3 flex flex-wrap items-center gap-4">
                <button onclick="openDaftarModal()"
                    ...
                    <span class="relative z-10">{{ __('achievements.hero_btn_daftar') }}</span>
                ...
                <a href="#achievements"
                    ...
                    <span>{{ __('achievements.hero_btn_lihat') }}</span>
            ...
            <div class="hero-fade hero-delay-4 flex flex-wrap gap-10 mt-16 pt-10 border-t border-white/10">
                {{-- Stats --}}
                @foreach ([['3', __('achievements.stat_lokasi')], ['9', __('achievements.stat_tahun')], ['100+', __('achievements.stat_atlet')]] as [$num, $label])
                    <div>
                        <p class="text-2xl font-extrabold text-white">{{ $num }}</p>
                        <p class="text-xs text-gray-400 font-medium mt-0.5 uppercase tracking-wider">{{ $label }}
                        </p>
                    </div>
                @endforeach
            </div>
    ...
    <div
        class="absolute bottom-10 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-2 hero-fade hero-delay-4">
        <span class="text-[10px] text-gray-400 uppercase tracking-widest">{{ __('achievements.scroll') }}</span>
