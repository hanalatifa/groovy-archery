<section id="athlets" class="py-24 bg-white dark:bg-slate-900 transition-colors duration-300">
    ...
    <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 px-6">
        ...
        @forelse ($atlets as $index => $atlet)
            ...
            @if($atlet->status == 'approved')
                ...
                <div class="athlete-card bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 overflow-hidden hover:shadow-lg transition duration-300 flex flex-col hover:scale-[1.02] {{ $index >= 4 ? 'hidden' : '' }}">
                    ...
                    <div class="p-6 flex flex-col flex-grow">
                        ...
                        <p class="text-gray-600 dark:text-gray-300 text-sm mb-6 flex-grow">
                            {{ Str::limit($atlet->deskripsi, 80) ?? __('athlets.default_athlete_desc') }}
                        </p>
        ...
        @empty
            <div class="col-span-full text-center py-20 bg-gray-50 dark:bg-slate-800/50 rounded-3xl border-2 border-dashed border-gray-100 dark:border-slate-700">
                <p class="text-gray-400 dark:text-slate-500 italic">
                    {{ __('athlets.empty') }}
                </p>
            </div>
        @endforelse
    ...
    @if(count($atlets->where('status', 'approved')) > 4)
        <div class="text-center mt-16" id="see-all-athletes-wrapper">
            <button onclick="showAllAthletes()"
                ...
                {{ __('athlets.btn_see_all') ?? 'See All Athletes' }}
            </button>
        </div>
    @endif
...
function showAllAthletes() {
    document.querySelectorAll('.athlete-card.hidden').forEach(card => {
        card.classList.remove('hidden');
    });
    document.getElementById('see-all-athletes-wrapper').classList.add('hidden');
}
    ...
    Swal.fire({
        ...
        html: `
            <div class="flex flex-col md:flex-row text-left overflow-hidden rounded-xl">
                <div class="w-full md:w-[45%] min-h-[300px] md:min-h-[450px]"
                     style="background: url('${foto}') center/cover no-repeat;">
                </div>
                <div class="w-full md:w-[55%] p-8 md:p-10">
                    <h2 class="text-2xl font-extrabold mb-8 ${isDark ? 'text-white' : 'text-gray-900'}">
                        {{ __('athlets.modal_title') }}
                    </h2>
                    <div class="grid grid-cols-2 gap-6 mb-8">
                        <div>
                            ...
                            <p class="text-base font-semibold ${isDark ? 'text-blue-100' : 'text-gray-800'}">
                                ${nama}
                            </p>
                        </div>
                        <div>
