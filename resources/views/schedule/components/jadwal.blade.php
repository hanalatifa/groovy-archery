    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="py-16 text-center transition-colors duration-300">
            <p class="text-[#2b459a] dark:text-blue-400 font-bold uppercase tracking-[5px] text-[10px]">
                {{ __('schedule.schedule_badge') }}</p>
            <h2 class="text-4xl md:text-5xl font-bold mt-2 text-gray-900 dark:text-white">
                {{ __('schedule.schedule_title') }}</h2>
            <p class="text-gray-500 dark:text-slate-400 mt-4 text-sm max-w-md mx-auto">
                {{ __('schedule.schedule_description') }}</p>
        </div>
        ...
        <div class="flex flex-wrap justify-center gap-2.5 mb-10">
            <button data-filter="all"
                ...
                <span class="w-2 h-2 rounded-full bg-white/60"></span> {{ __('schedule.all_locations') }}
            </button>
        ...
        <div class="overflow-x-auto border border-gray-100 dark:border-slate-700 shadow-sm">
            <div class="min-w-[640px]">
                ...
                <div class="grid border-b border-gray-100 dark:border-slate-700"
                    ...
                    <div
                        class="px-5 py-4 bg-gray-50 dark:bg-slate-800 text-[10px] font-black text-gray-300 dark:text-slate-500 uppercase tracking-widest">
                        {{ __('schedule.location') }}
                    </div>
                    @foreach ([__('schedule.monday'), __('schedule.tuesday'), __('schedule.wednesday'), __('schedule.thursday'), __('schedule.friday'), __('schedule.saturday'), __('schedule.sunday')] as $i => $day)
                        <div
                            class="px-2 py-4 text-center bg-gray-50 dark:bg-slate-800 border-l border-gray-100 dark:border-slate-700">
                            <span
                                class="text-[10px] font-black uppercase tracking-wider text-gray-500 dark:text-slate-400">
                                {{ $day }}
                            </span>

                            @if (in_array($day, [__('schedule.saturday'), __('schedule.sunday')]))
                ...
                <div class="sched-row grid items-stretch border-b border-gray-100 dark:border-slate-700 transition-all duration-300 bg-white dark:bg-slate-900"
                    ...
                    <div
                        class="px-5 py-5 border-r border-gray-100 dark:border-slate-700 flex flex-col justify-center gap-1">
                        <span class="w-2 h-2 rounded-full bg-indigo-500"></span>
                        <span
                            class="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-wider leading-tight">GRTP<br>Sunter</span>
                    </div>
                    @foreach (['Sen', 'Sel', 'Rab', 'Kam', 'Jum'] as $d)
                        <div
                            class="border-l border-gray-100 dark:border-slate-700 px-2 py-5 flex items-center justify-center">
                    ...
                    <div class="border-l border-gray-100 dark:border-slate-700 px-2 py-4 flex flex-col gap-1.5 justify-center"
                        ...
                        <div class="px-2 py-2 text-center" style="background: rgba(99,102,241,0.1);">
                            <p
                                class="text-[10px] font-black text-indigo-700 dark:text-indigo-300 !text-indigo-700 dark:!text-indigo-300">
                                08.00–10.00</p>
                            <p class="text-[9px] text-indigo-400 mt-0.5 !text-indigo-400">{{ __('schedule.session_1') }}
                            </p>
                        </div>
                        <div class="px-2 py-2 text-center" style="background: rgba(99,102,241,0.07);">
                            <p
                                class="text-[10px] font-black text-indigo-600 dark:text-indigo-400 !text-indigo-600 dark:!text-indigo-400">
                                10.00–12.00</p>
                            <p class="text-[9px] text-indigo-300 mt-0.5 !text-indigo-300">{{ __('schedule.session_2') }}
                            </p>
                        </div>
                    </div>
                    {{-- Minggu --}}
                ...
                <div class="sched-row grid items-stretch border-b border-gray-100 dark:border-slate-700 transition-all duration-300 bg-white dark:bg-slate-900"
                    ...
                    <div
                        ...
                        <span
                            class="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-wider leading-tight">BMW<br>JIS</span>
                    </div>
                    {{-- Senin --}}
                    <div
                        class="border-l border-gray-100 dark:border-slate-700 px-2 py-5 flex items-center justify-center">
                    ...
                    @foreach (['Sel', 'Rab', 'Kam'] as $d)
                        <div class="border-l border-gray-100 dark:border-slate-700 px-2 py-4 flex items-center justify-center"
                            ...
                            <div class="px-2 py-2 text-center w-full" style="background: rgba(16,185,129,0.1);">
                                <p
                                    class="text-[10px] font-black text-emerald-700 dark:text-emerald-300 !text-emerald-700 dark:!text-emerald-300">
                                    15.30–18.00</p>
                                <p class="text-[9px] text-emerald-400 mt-0.5 !text-emerald-400">
                                    {{ __('schedule.afternoon') }}</p>
                            </div>
                        </div>
                    @endforeach
                    {{-- Jumat --}}
                ...
                <div class="sched-row grid items-stretch transition-all duration-300 bg-white dark:bg-slate-900"
                    ...
                    <div
                        ...
                        <span
                            class="text-[10px] font-black text-amber-600 dark:text-amber-400 uppercase tracking-wider leading-tight">Yon<br>Arhanud</span>
                    </div>
                    {{-- Senin --}}
                    <div class="border-l border-gray-100 dark:border-slate-700 px-2 py-4 flex items-center justify-center"
                        ...
                        <div class="px-2 py-2 text-center w-full" style="background: rgba(245,158,11,0.1);">
                            <p
                                class="text-[10px] font-black text-amber-700 dark:text-amber-300 !text-amber-700 dark:!text-amber-300">
                                16.00–17.30</p>
                            <p class="text-[9px] text-amber-400 mt-0.5 !text-amber-400">{{ __('schedule.afternoon') }}
                            </p>
                        </div>
                    </div>
                    {{-- Sel Rab Kam --}}
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 mt-8">
            <div class="flex flex-wrap gap-6">
                <div class="flex items-center gap-2">
                    ...
                    <span
                        class="text-xs text-gray-400 dark:text-slate-500 font-semibold !text-gray-400 dark:!text-slate-500">GRTP
                        Sunter Mall</span>
                </div>
                <div class="flex items-center gap-2">
                    <div class="w-3 h-3 rounded-full bg-emerald-500"></div>
                    <span
                        class="text-xs text-gray-400 dark:text-slate-500 font-semibold !text-gray-400 dark:!text-slate-500">BMW
                        JIS</span>
                </div>
                <div class="flex items-center gap-2">
                    <div class="w-3 h-3 rounded-full bg-amber-500"></div>
                    <span
                        class="text-xs text-gray-400 dark:text-slate-500 font-semibold !text-gray-400 dark:!text-slate-500">Yon
                        Arhanud</span>
                </div>
            </div>
            <div class="flex items-center gap-1.5 px-4 py-2 dark:bg-indigo-900/20"
                ...
                <span
                    class="text-[10px] text-indigo-400 dark:text-indigo-400 font-bold !text-indigo-400">{{ __('schedule.weekend_note') }}</span>
            </div>
        </div>
...
<script>
    document.querySelectorAll('.sched-pill').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            btn.classList.remove('border-gray-100', 'bg-white', 'text-gray-500', 'border-slate-700',
