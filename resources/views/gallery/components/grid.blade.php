...
<section class="bg-gray px-6 py-16" id="gallery">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-4" id="glryGrid">
            @foreach ($docs as $i => $doc)
                ...
                <div class="glry-item break-inside-avoid mb-4 group relative overflow-hidden rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300"
                    data-index="{{ $i }}" data-kategori="{{ $kategoriSlug }}"
                    style="animation-delay:{{ $i * 50 }}ms; cursor: pointer; position: relative;">

                    <div
                        class="relative overflow-hidden rounded-2xl {{ $ukuran === 'tall' ? 'aspect-[3/5]' : ($ukuran === 'wide' ? 'aspect-[16/9]' : 'aspect-[4/3]') }}">

                        <div class="absolute top-3 left-3 z-20">
                            <span
                                class="bg-[#2b459a] text-white text-[9px] font-black uppercase tracking-[2px] px-3 py-1.5 rounded-lg shadow-lg">
                                {{ $doc->kategori }}
                            </span>
                        </div>
                        ...
                        <div
                            class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-400">
                        </div>
        ...
        <div id="glryEmpty" class="{{ count($docs) > 0 ? 'hidden' : '' }} text-center py-24 text-gray-400">
            ...
            <p class="text-sm font-black uppercase tracking-widest">{{ __('gallery.grid_empty') }}</p>
            </p>
        </div>
...
<div id="modalGallery" class="hidden fixed inset-0 w-full h-full bg-black/95 flex-col items-center justify-center p-4"
    style="z-index:999999; display:none;">

    <button id="closeBtn" class="fixed top-6 right-6 p-4 text-white/50 hover:text-white hover:bg-white/20 rounded-full"
    ...
    <div class="flex flex-col items-center w-full max-w-4xl gap-6">
        <div class="w-full flex justify-center">
            <img id="imgGallery" class="max-w-full max-h-[65vh] object-contain shadow-2xl border border-white/10">
        </div>
        ...
        <div class="w-full text-left px-4">
            ...
            <h2 id="judulGallery" class="text-white text-2xl md:text-4xl font-black mb-3"></h2>

            <p id="deskGallery" class="text-white/60 text-sm md:text-base italic"></p>
        </div>
    </div>
</div>
        ...
        document.querySelectorAll('.glry-filter').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                document.querySelectorAll('.glry-filter').forEach(b => {
                    b.classList.remove('active-filter', 'bg-[#2b459a]', 'text-white',
                        'border-[#2b459a]');
                    b.classList.add('bg-white', 'text-gray-400', 'border-gray-200');
                });

                this.classList.add('active-filter', 'bg-[#2b459a]', 'text-white',
                    'border-[#2b459a]');
